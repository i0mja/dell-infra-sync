...
const Layout = () => {
  ...
  const NavLinks = () => <>
      ...
      return <Button key={item.name} variant={isActive ? "secondary" : "ghost"} className={cn("w-full justify-start transition-all duration-200 truncate", isActive && "bg-secondary")} onClick={() => {
        navigate(item.href);
        setMobileOpen(false);
      }} title={item.name}>
      ...
      <Collapsible open={settingsOpen}>
        <CollapsibleTrigger asChild>
          <Button variant={location.pathname === '/settings' ? "secondary" : "ghost"} className={cn("w-full justify-start transition-all duration-200", location.pathname === '/settings' && "bg-secondary")} onClick={() => {
          ...
          setSettingsOpen(!settingsOpen);
            ...
            <ChevronRight className={cn("h-4 w-4 transition-transform duration-300 ease-in-out", settingsOpen && "rotate-90")} />
          </Button>
        </CollapsibleTrigger>
        <CollapsibleContent className="space-y-1 mt-1">
          {settingsNavigation.map(item => {
          const isActive = location.pathname === '/settings' && (item.href.includes(`tab=${activeTab}`) || !activeTab && item.href.includes('tab=appearance'));
          return <Button key={item.name} variant={isActive ? "secondary" : "ghost"} className={cn("w-full justify-start pl-10 text-sm transition-all duration-200 truncate", isActive && "bg-muted text-foreground font-medium")} onClick={() => {
            navigate(item.href);
            setMobileOpen(false);
          }} title={item.name}>
  ...
  const useEdgeToEdgeLayout = edgeToEdgeRoutes.some(path => location.pathname.startsWith(path));
  const containerClasses = cn("w-full", useEdgeToEdgeLayout ? "max-w-full px-0 pb-0 pt-0 h-full" : "mx-auto max-w-screen-2xl px-4 py-6 sm:px-6 lg:px-8");
  return <div className="flex h-screen bg-background">
      ...
      <aside className="hidden w-52 border-r bg-card md:block">
        <div className="flex h-full flex-col">
          ...
          <ScrollArea className="flex-1">
            <nav className="space-y-0.5 p-3 pt-px px-0">
              <NavLinks />
            </nav>
          </ScrollArea>
      ...
      <div className="flex flex-col flex-1">
        ...
        {/* Main Content */}
        <main className={cn("flex-1 bg-background", useEdgeToEdgeLayout ? "overflow-hidden" : "overflow-auto")}>
          <div className={containerClasses}>
            <div className={cn("animate-in fade-in duration-300", useEdgeToEdgeLayout && "h-full")}>
